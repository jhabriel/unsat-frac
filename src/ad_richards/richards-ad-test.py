...
def sat(p):
    return sr_vg + (1 - sr_vg) * (1 + (a_vg * (p ** 2) ** 0.5) ** n_vg) ** (
        -(1 - 1 / n_vg)
    )
...
def flux_ex(x, y, t):
    return [
        -mobility(p_ex(x, y, t)) * dpdx_ex(x, y, t),
        -mobility(p_ex(x, y, t)) * dpdy_ex(x, y, t),
    ]
...
gb = pp.meshing.cart_grid(
    [], nx=[4 * 2 ** refinementlvl, 4 * 2 ** refinementlvl], physdims=[1, 1]
)
...
if weighting_type == "harmonic":
    ...
    face_transmissibility = upwind(
        mobilityAd(p), p, mobilityAd(dir_bc_flow), dir_bc_flow
    ) * harmAvg(perm)
elif weighting_type == "lazy upstream":
    face_transmissibility = upwind(
        mobilityAd(p.previous_iteration()), p, mobilityAd(dir_bc_flow), dir_bc_flow
    ) * harmAvg(perm)
else:
...
for n in range(1, num_time_steps + 1):
    ...
    flow_param["bc_values"][neu_faces] = sum(
        [fnormal[i] * flux_bc[i] for i in range(0, 2)]
    )

    # Source term
    cc = g.cell_centers
    x_ad, y_ad, t_ad = initLocalAdArrays(
    ...
    while iteration_counter <= 30 and not (rel_res < 1e-12 or residual_norm < 1e-12):
        ...
        if sat_linearization == "newton":
            flow_momentum_eq = mass_ad.mass * satAd(p) + dt * div * tpfa_ad.flux(
                face_transmissibility, p, bc_flow
            )
            ...
            flow_momentum_eq = (
                dt * div * tpfa_ad.flux(face_transmissibility, p, bc_flow)
        ...
        flow_rhs = (
            mass_ad.mass * satAd(p.previous_timestep()) + dt * mass_ad.mass * source
        )

        # Flow equation
        flow_eq = pp.ad.Expression(
            flow_momentum_eq - flow_rhs, dof_manager=dof_manager, name="flow equation"
        )
        ...
        d[pp.STATE][pp.ITERATE][pressure_variable] = acceleration.apply(
            d[pp.STATE][pp.ITERATE][pressure_variable],
            pressure_increment,
            iteration_counter,
        )
