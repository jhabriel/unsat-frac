...
def model(gb, method, krw_order):
    ...
    for e, d in gb.edges():
        ...
        mg = d['mortar_grid']
        h_mortar = np.min(mg.cell_volumes)
    
        # Set the normal diffusivity
        data = {"normal_diffusivity": h_mortar ** (-krw_order)}
    ...
    def get_cc_reconvel(estimates, cell_idx_list):
        ...
        for g, d in gb:
            ...
            if g.dim == 2:
                ...
                for idx in cell_idx_list:
                    ru_cc_2d_x += (
                        u[0] * x[0].reshape(g.num_cells, 1) 
                        + u[1]
                        ) * idx.reshape(g.num_cells, 1)
                    ru_cc_2d_y += (
                        u[0] * x[1].reshape(g.num_cells, 1) 
                        + u[2]
                        ) * idx.reshape(g.num_cells, 1)
    ...
    def compute_pressure_2d_true_error(g, d, estimates, gradp2d_numpy_list, cell_idx_list):
        """
    ...
    def compute_velocity_mortar_true_error_k(d_e, estimates):
        """
        Computes the true "velocity" error for the mortar grid 
        ...
        lmbda = d_e[pp.STATE][estimates.lam_name].copy()
        mortar_flux = lmbda / V
        lbmda_diff = (np.sqrt(V) * (1.0 - mortar_flux) ) ** 2
        true_error_mortar = lbmda_diff * V
    ...
    true_pressure_error_1d = compute_pressure_1d_true_error(
        g_1d, d_1d, estimates
    )
    true_pressure_error_mortar = compute_velocity_mortar_true_error_k(
        d_e, estimates
    )
    ...
    true_velocity_error_1d = compute_velocity_1d_true_error(
        g_1d, d_1d, estimates
    )
    true_velocity_error_mortar = compute_velocity_mortar_true_error(
        d_e, estimates
    )
