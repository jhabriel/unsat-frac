...
tsc = pp.TimeSteppingControl(
    schedule=[0, 4*pp.HOUR],
    dt_init=1,
    dt_min_max=(0.01, 0.25*pp.HOUR),
    iter_max=70,
    iter_optimal_range=(30, 40),
...
exporter_ghost.write_vtu([bulk_var, "is_sat"], time_step=0)
...
while time < final_time:
    ...
    while recompute_solution:
        ...
        while iteration_counter <= max_iter and not residual_norm < 1e-5:
            ...
            print("time", time, "iter", iteration_counter, "res", residual_norm, "rel_res",
                  rel_res, "dt", dt)
        ...
        if iteration_counter >= max_iter + 1 and recompute_div_attempts < 10:
            ...
            set_iterate_to_state(gb, bulk_var, fracture_var,
                                 mortar_var)  # set iterate to state
        ...
        if control_faces.sum() == 0 and is_mortar_conductive.sum() > 0:
            ...
            set_iterate_to_state(gb, bulk_var, fracture_var,
                                 mortar_var)  # return to previous state
        else:
