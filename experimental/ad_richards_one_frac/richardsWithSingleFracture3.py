...
# Otherwise, numpy will broadcast the expression leading to and undesirable output format
...
# Interface flux
lam_1ph = robin.mortar_scaling * (tr_psi_bulk - psi_frac)
lam_mph = krw_interface_ad * lam_1ph
lam = H_eps(tr_psi_bulk) * lam_mph
interface_flux_eq = lam + robin.mortar_discr * lmbda
...
for n in range(1, num_time_steps + 1):
    ...
    while iteration_counter <= 1000 and not (rel_res < 1e-6 or residual_norm < 1e-6):
