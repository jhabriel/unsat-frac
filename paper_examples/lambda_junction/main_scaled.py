...
gfo = GridGenerator(
    ...
    #mesh_args={"mesh_size_frac": 5, "mesh_size_bound": 7.5},
...
TS = (1 / 3600)  # Time Scale

# %% Time parameters
schedule = list(np.linspace(0, 4000 * TS, 100))
tsc = pp.TimeSteppingControl(
    schedule=schedule,
    dt_init=0.1 * TS,
    dt_min_max=(0.001 * TS, 3600 * TS),
    iter_max=30,
...
for g, d in gb:
    ...
    if g.dim == gb.dim_max():
        ...
        bc_values[top] = -0.05 * (1 / TS)

        # Saturated hydraulic conductivity
        K_SAT_TOP_LAYER = (soil_catalog["sandy_loam"].K_s/3600) * (1 / TS)  # [cm/s]
        K_SAT_MID_LAYER = (soil_catalog["sandy_clay_loam"].K_s/3600) * (1 / TS)  # [cm/s]
        K_SAT_BOT_LAYER = (soil_catalog["loamy_sand"].K_s / 3600) * (1 / TS) # [cm/s]
        ...
        specified_parameters: dict = {
            ...
            "alpha_vG": ALPHA_VG,  # van Genuchten parameter [1/cm]
            "n_vG": N_VG,  # van Genuchten parameter [-]
            "m_vG": 1 - N_VG ** (-1),  # van Genuchten parameter [-]
            "time_step": tsc.dt,  # [s]
        }
...
for e, d in gb.edges():
    ...
    else:  # 0-dimensional interfaces should be always impervious to flow
        data = {
