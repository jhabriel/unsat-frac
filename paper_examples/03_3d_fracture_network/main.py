...
gfo = GridGenerator(
    mesh_args={"mesh_size_frac": 10, "mesh_size_bound": 10, "mesh_size_min": 1},
    csv_file="network.csv",
    domain={"xmin": 0, "ymin": 0, "zmin": 0, "xmax": 100, "ymax": 100, "zmax": 100},
    #constraints=[],
)
...
#
# # %% Time parameters
# schedule = list(np.linspace(0, 400, 60))
...
#         bc_values[top] = -0.01  # -15 (pressure_head) + y_max (elevation_head)
#
...
#
# #water_table_bottom = [d_frac_bottom[pp.STATE][node_var][0]]
# #water_table_top = [d_frac_top[pp.STATE][node_var][0]]
...
#     #d_1d_bot = gb.node_props(lfn_list[0])
#     #d_1d_top = gb.node_props(lfn_list[1])
#     vol_top = vol(h_lfn.evaluate(dof_manager).val)[0]
#     vol_left = vol(h_lfn.evaluate(dof_manager).val)[1]
#     vol_right = vol(h_lfn.evaluate(dof_manager).val)[2]
#     vol_top_max = 40 * 0.1
#     vol_left_max = 42.42640687 * 0.1
#     vol_right_max = 31.6227766 * 0.1
#     if (vol_left_max - vol_left) > vol_top and (vol_right_max - vol_right) > vol_top:
#         vol_left = vol_left + vol_top/2
#         vol_right = vol_right + vol_top/2
#         vol_top = 0
#         # Correct values of hydraulic head
#         d_top[pp.STATE][pp.ITERATE][node_var] = (vol_top / 0.1) + 50
#         d_left[pp.STATE][pp.ITERATE][node_var] = (vol_left / 0.1) + 20
#         d_right[pp.STATE][pp.ITERATE][node_var] = (vol_right / 0.1) + 20
#     else:
#         pass
#         # if np.abs(vol_bot_max - vol_bot) <= 1e-5:
#         #     vol_top = vol_top
#         #     vol_bot = vol_bot_max
#         # else:
#         #     vol_top = vol_top - (vol_bot_max - vol_bot)
#         #     vol_bot = vol_bot_max
#         # # Correct values of hydraulic head
#         # d_1d_top[pp.STATE][pp.ITERATE][node_var] = (vol_top / 0.1) + 50
#         # d_1d_bot[pp.STATE][pp.ITERATE][node_var] = d_1d_top[pp.STATE][pp.ITERATE][node_var]
#
#
