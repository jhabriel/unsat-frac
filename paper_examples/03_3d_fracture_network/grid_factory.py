...
class GridGenerator:
    ...
    def __init__(
        ...
        Parameters:
            ...
            csv_file (str): path to the CSV file containing the fracture network and mesh
                constraints.
            domain (dict): dictionary containing the bounding box of the 3d domain. Expected
    ...
    def get_grid_buckets(self) -> Tuple[pp.GridBucket, pp.GridBucket]:
        ...
        # Retrieve the 2D fractures
        #frac = np.loadtxt(self.file, delimiter=",", dtype=float, skiprows=1)
        #nodes_x = frac[::3]
        #nodes_y = frac[1::3]
        #nodes_z = frac[2::3]
        #frac_nodes = [np.row_stack((nodes_x, nodes_y, nodes_z))]

        # Create a list containing all the ghost fracture grids (in this case only 1)
        ghost_frac_list = [g for g, _ in gb_ghost if g.dim == 2]
        ...
        mono_fracs: List[pp.CartGrid] = []
        for g in ghost_frac_list:
            # xmin: float = np.min(g.nodes[0])
            # xmax: float = np.max(g.nodes[0])
            # ymin: float = np.min(g.nodes[1])
            # ymax: float = np.max(g.nodes[1])
            # zmin: float = np.min(g.nodes[2])
            # zmax: float = np.max(g.nodes[2])
            # physdims: dict = {
            #     "xmin": xmin,
            #     "xmax": xmax,
            #     "ymin": ymin,
            ...
            #     "zmax": zmax,
            # }
            nodes = np.stack((g.nodes[0][:4], g.nodes[1][:4], g.nodes[2][:4]))
            frac_nodes.append(nodes)
            mono_fracs.append(pp.CartGrid(nx=[1, 1]))
        ...
        #mono_fracs = [pp.CartGrid(np.array([1, 1]))]
        ...
        frac_list = [g for g, _ in gb if g.dim <= 2]
