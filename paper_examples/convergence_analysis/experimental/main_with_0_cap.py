...
# # Plot exact solution
# g_bulk = ghost_bulk_list[0]
# psi_ex = ex.rock_pressure_head(g_bulk, 0.5)
# h_ex = ex.rock_hydraulic_head(g_bulk, 0.5)
# theta_ex = ex.rock_water_content(g_bulk, 0.5)
# krw_ex = ex.rock_relative_permeability(g_bulk, 0.5)
# f_ex = ex.rock_source(g_bulk, 0.5)
#
# pp.plot_grid(g_bulk, psi_ex, plot_2d=True, linewidth=0, title="Pressure head")
# pp.plot_grid(g_bulk, theta_ex, plot_2d=True, linewidth=0, title="Water content")
# pp.plot_grid(g_bulk, krw_ex, plot_2d=True, linewidth=0, title="Relative permeability")
# pp.plot_grid(g_bulk, f_ex, plot_2d=True, linewidth=0, title="Source term")


# ccx = np.logical_and(g_bulk.cell_centers[1] < 0.501, g_bulk.cell_centers[1] > 0.499)
# plt.plot(
#     g_bulk.cell_centers[0][ccx],
#     psi_ex[ccx],
#     linewidth=0,
#     marker="*"
# )
...
for e, d in gb.edges():
    ...
    exact_hyd_head_jump = 0.75 - (psi_l + zeta_mortar)  # exact jump
...
def smc(pressure_head):
    ...
    if isinstance(pressure_head, pp.ad.Ad_array):
        is_unsat = 1 - pp.ad.heaviside(pressure_head.val, 1)
        is_sat = 1 - is_unsat
        smc_val = (1 - pressure_head) ** (-2) * is_unsat + 1 * is_sat
    else:
        is_unsat = 1 - pp.ad.heaviside(pressure_head, 1)
        is_sat = 1 - is_unsat
...
conserv_frac_eq = accum_frac - tsc.dt * sources_from_mortar
...
h_frac_mpfa = h_frac.evaluate(dof_manager).val - (np.mean(pressure_threshold) + 0.25)
