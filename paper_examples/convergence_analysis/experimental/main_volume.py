...
# gfo = GridGenerator(
#     mesh_args={"mesh_size_frac": 0.005, "mesh_size_bound": 0.005},
...
# for g, d in gb:
#     if g.dim == 2:
#         d[pp.PRIMARY_VARIABLES] = {bulk_var: {"cells": 1}}
#     else:
#         d[pp.PRIMARY_VARIABLES] = {frac_var: {"cells": 1}}
#
# for _, d in gb.edges():
...
# for g, d in gb:
#     if g.dim == gfo.dim:
#         pp.set_state(d, state={bulk_var: -1 + d[pp.PARAMETERS][param_key]["elevation"]})
#         pp.set_iterate(d, iterate={bulk_var: d[pp.STATE][bulk_var]})
#     else:
#         pp.set_state(d, state={frac_var: 0})  # 0 initial volume
#         pp.set_iterate(d, iterate={frac_var: d[pp.STATE][frac_var]})
#
# for _, d in gb.edges():
#     mg = d["mortar_grid"]
#     pp.set_state(d, state={edge_var: np.zeros(mg.num_cells)})  # impervious barrier
...
# h_bulk = equation_manager.merge_variables([(g, bulk_var) for g in bulk_list])
# vol_frac = equation_manager.merge_variables([(g, frac_var) for g in frac_list])
...
# if linearization == "newton":
#     accum_frac_active = vol_frac
#     accum_frac_inactive = vol_frac_n * (-1)
# elif linearization == "modified_picard":
#     ...
# elif linearization == "l_scheme":
#     ...
